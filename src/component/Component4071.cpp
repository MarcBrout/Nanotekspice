...
nts::Tristate nts::Component4071::Compute(size_t pin_num_this) {
    ...
    if (isInPinList(InPins, pin_num_this)) {
        ...
        switch (pin_num_this) {
            case 3 : state = Pins[3] = Gates::_or(Pins[1], Pins[2]); break;
            case 4 : state = Pins[4] = Gates::_or(Pins[5], Pins[6]); break;
            case 10 : state = Pins[10] = Gates::_or(Pins[8], Pins[9]); break;
            case 11 : state = Pins[11] = Gates::_or(Pins[12], Pins[13]); break;
